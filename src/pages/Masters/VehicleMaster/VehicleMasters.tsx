// VehicleMastersPage.tsx - Complete Implementation
...
import { transformVehicleMastersData } from "../../../components/CustomSummary/utils/summaryDataTransformers";
...
interface VehicleMasterSummaryData {
...
const VehicleMasters: React.FC = () => {
  ...
  const [summaryData, setSummaryData] = useState<VehicleMasterSummaryData | null>(
    null
  );
  ...
  useEffect(() => {
    const savedFilters = localStorage.getItem("vehicle_masters_filters");
    if (savedFilters) {
      try {
  ...
  useEffect(() => {
    if (activeFilters.length > 0) {
      localStorage.setItem("vehicle_masters_filters", JSON.stringify(activeFilters));
    } else {
      localStorage.removeItem("vehicle_masters_filters");
    }
  }, [activeFilters]);
  ...
  const loadSummaryData = useCallback(async () => {
    ...
    try {
      const data = await vehicleMasterServices.getFilterSummary();
      setSummaryData(data);
    } catch (error: any) {
  ...
  // Load vehicle masters method
  const loadVehicleMasters = useCallback(
    async (
      ...
      try {
        ...
        const result = await vehicleMasterServices.getAll(
          page,
          limit,
          search,
          sortField,
  ...
  const summaryCards = useMemo((): SummaryCard[] => {
    ...
    return transformVehicleMastersData(summaryData, {
      totalIcon: React.createElement(FaTruck),
      driverIcon: React.createElement(FaUserTie),
    });
  }, [summaryData]);

  // Summary configuration
  const summaryConfig = useMemo(() => {
    return getConfigPreset("vehicleMasters", {
  ...
  const handleSummaryCardClick = useCallback((card: SummaryCard) => {
    ...
    switch (card.id) {
      case "vehicle-brands":
        console.log("Navigate to vehicle brands view");
        break;
      case "vehicle-types":
      ...
      case "vehicle-statuses":
        console.log("Navigate to vehicle status view");
        break;
      case "total-vehicle-masters":
        console.log("Show all vehicle masters");
        break;
      case "inactive-vehicle-masters":
        console.log("Show inactive vehicle masters");
        break;
      default:
        break;
    }
  ...
  const handlePageSizeChange = useCallback(
    (size: number) => {
      ...
      if (size === 0) {
        ...
        loadVehicleMasters(searchValue, 1, 999999); // Large number to get all records
      } else {
  ...
  const handleRowAction = useCallback(
    (action: "view" | "edit" | "delete", rowId: string | number) => {
      const selectedVehicleMaster = vehicleMasters.find(
        (vehicleMaster) => vehicleMaster.id === rowId
      );

      switch (action) {
        case "view":
          console.log("View vehicle master:", selectedVehicleMaster);
          navigate(`${urls.vehicleMastersViewPath}/${rowId}`, {
            state: { vehicleMasterData: selectedVehicleMaster },
          });
          break;
        case "edit":
          navigate(`${urls.editVehicleMasterViewPath}/${rowId}`, {
            state: { vehicleMasterData: selectedVehicleMaster },
          });
          break;
        case "delete":
          handleDeleteVehicleMaster(rowId);
          break;
      }
    },
    [vehicleMasters, navigate]
  );

  // Handle delete vehicle master
  const handleDeleteVehicleMaster = useCallback(
    async (id: string | number) => {
      if (window.confirm("Are you sure you want to inactivate this vehicle master?")) {
        try {
          setLoading(true);
  ...
  // Handle add vehicle master
  const handleAddVehicleMaster = useCallback(() => {
  ...
  const handleImport = useCallback(
    async (file: File) => {
      try {
        setLoading(true);
        const result = await vehicleMasterServices.import(file);
        ...
        // Reload vehicle masters
        await loadVehicleMasters();
      } catch (error: any) {
        console.error("Error importing vehicle masters:", error);
        toast.error(error.message || "Failed to import vehicle masters");
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleGetFilterOptions = useCallback(
    async (field: string, searchText?: string): Promise<FilterOption[]> => {
      try {
        return await vehicleMasterServices.getFilterOptions(field, searchText);
      } catch (error: any) {
