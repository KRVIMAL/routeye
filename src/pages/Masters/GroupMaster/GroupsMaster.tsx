...
const GroupsMaster: React.FC = () => {
  ...
  useEffect(() => {
    if (activeFilters.length > 0) {
      localStorage.setItem("groups_master_filters", JSON.stringify(activeFilters));
    } else {
  ...
  const handleExport = useCallback(async () => {
    try {
      setLoading(true);
      const blob = await groupsMasterServices.export(activeFilters);

      // Create download link
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `groups_master_export_${
