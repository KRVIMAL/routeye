...
const DeviceOnboarding: React.FC = () => {
  ...
  const [summaryData, setSummaryData] =
    useState<DeviceOnboardingSummaryData | null>(null);
  ...
  const handleExport = async (format: ExportFormat) => {
    try {
      await exportService.exportData(
        `${urls.deviceOnboardingViewPath}/export`,
        format,
        "device-onboarding"
      );

      toast.success(
        `Device Onboarding exported successfully as ${format.toUpperCase()}`
      );
    } catch (error: any) {
  ...
  const handleGetFilterOptions = useCallback(
    async (field: string, searchText?: string): Promise<FilterOption[]> => {
      try {
        return await deviceOnboardingServices.getFilterOptions(
          field,
          searchText
        );
      } catch (error: any) {
